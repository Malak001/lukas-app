...
export async function submitStage1Intro(essayText: string) {
  try {
    ...
    const nextStage = Math.max(existingStage?.current_stage ?? 1, 2);
    ...
    console.error("submitStage1Intro: error", err instanceof Error ? err.message : err);
    throw err;
  }
}
